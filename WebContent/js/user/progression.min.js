(function(e, t, n, r) {
	...
	o.prototype = {
		init : function() {
			...
			data_prog = this.$elem.css("position", "relative").find(
					"[data-progression]");
			...
			var i = this.options.tooltipAnimate ? "-webkit-transition: width .3s ease-in-out;-moz-transition: width .3s ease-in-out;-o-transition: width .3s ease-in-out;transition: width .3s ease-in-out;"
					: "";
			var s = this.options.tooltipPosition == "right" ? "border-color: transparent #"
					+ this.options.tooltipBackgroundColor
					+ " transparent transparent;"
					: "border-color: transparent transparent transparent #"
							+ this.options.tooltipBackgroundColor + "";
			marginRight = parseInt(this.options.tooltipWidth)
					+ parseInt(this.options.tooltipOffset);
			myhtml = e('<div class="syco_tooltip" data-tooltip="'
					+ thisid
					+ '" style="'
					+ n
					+ "padding:"
					...
					+ this.options.tooltipBackgroundColor
					+ ";"
					+ this.options.tooltipPosition
					+ ":-"
					+ marginRight
					+ "px;width:"
					+ this.options.tooltipWidth
					+ 'px" ><span class="triangle_'
					+ this.options.tooltipPosition
					+ '" style="'
					+ s
					+ '"></span><p style="'
					...
					+ '"><span class="tooltip_helper"><span data-index="1" >1</span>/'
					+ items + "</span> " + firsthelper
					+ '</p><div class="percentagebar" style="' + $display
					+ "background:#" + this.options.progressBarBackground
					+ '""><div class="percentagebarinner" style="' + i
					+ "background:#" + this.options.progressBarColor
					+ '"></div><span class="percent" ' + $display
					+ ">0%</span></div></div>");
			...
			data_prog.each(function() {
				...
				n.bind("live focus change", function() {
					thisprogressionlength = n.parent().parent().find(
							"[data-progression]").length;
					alldataprogression = n.parent().parent().find(
							"[data-progression]");
					...
					index = parseInt(e("#" + thisid2)
							.find("[data-progression]").index(n)) + 1;
					percentage = t(thisprogressionlength, index).toFixed(0);
					thistooltip.find("p").html(
							'<span class="tooltip_helper"><span data-index="1" >'
									+ index + "</span>/"
									+ thisprogressionlength + "</span> "
									+ thishelper).parent().find(
							".percentagebarinner").css("width",
							parseInt(percentage) + "%").next().html(
							parseInt(percentage) + "%");
